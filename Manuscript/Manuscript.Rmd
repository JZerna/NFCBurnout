...
abstract: |
  Burnout has become more prevalent, mainly in social jobs. While there is evidence that certain personality traits promote or protect against burnout, the role of dispositional cognitive motivation has been investigated only recently.
  ...
  We analyzed data of $N=180$ teachers to replicate previous results on a mediation of NFC and the burnout subscale reduced personal efficacy by habitual reappraisal, but not by habitual suppression or self-control.
  With our data, self-control became a significant mediator when taking teaching experience into account, but neither reappraisal nor suppression did.
  We also investigated whether NFC and burnout were associated via different ratios of demands and resources, and included other variables in an exploratory approach.
  The results indicated that teachers with higher NFC and self-control have lower burnout because they experience their resources as fitting to the demands.
  
keywords          : "behaviour, resources, demands, education, Covid-19"
wordcount         : "~ 5600"
...
Burnout is especially prevalent in social jobs such as healthcare or teaching because the worker is often in conflict between advocating for their client and meeting the goals set by the employer [@GrayStanley2011; @Lloyd2002].
...
This is supported by findings that NFC showed moderate to large negative associations with burnout scores in adults [@Fleischhauer2019], students [@Fleischhauer2019; @Naderi2018], and student teachers [@Grass2018].
...
One goal of this paper was to replicate the findings of Grass et al. [-@Grass2018] in a teacher sample using a multiple mediation model on cross-sectional self-report data of teachers.
...
To sum up, this study had three aims: Replicating findings of mediation between NFC and reduced personal efficacy in a teacher sample, investigating the impact of different demand-resource-ratios on the relationship between NFC and burnout, and exploring the impact of other variables such as perceived burden by the Covid-19 pandemic.
...
## Participants
We set out to recruit a sample of $N\ge287$ teachers as determined using the R package *pwr* [@Champely2020], assuming a small to medium correlation of $r=.20$ between the measures of interest and targeting at a power of $1-\beta=.80$ at  $\alpha=.01$.
...
Using the smallest standardized indirect effect from the mediation in Grass et al. [-@Grass2018] in a post hoc power analysis with *G\*Power* [@Faul2007;@Faul2009] yielded a power of $1-\beta=.85$ for linear multiple regression, given $\alpha=.05$, $N=180$, and $f^2=.05$.

## Material
All questionnaires were used in their German form.
Burnout was assessed using the 21-item Maslach Burnout Inventory [MBI, @Buessing1992], with items such as "I don't really care what happens to some recipients" and responses on a 7-point Likert scale from 0 (never) to 6 (every day).
Its subscales *emotional exhaustion*, *depersonalization*, and *reduced personal efficacy* showed acceptable internal consistency (Cronbach’s $\alpha>.70$) and retest reliabilities of $r_{tt}=.60$, $r_{tt}=.54$, and $r_{tt}=.57$, respectively, over the span of one year in teachers [@Maslach1997].
NFC was assessed with the 16-item short version of the German NFC scale (NCS) [@Bless1994], consisting of items (e.g., "Thinking is not my idea of fun", recoded) that are answered on a 7-point Likert scale ranging from -3 (completely disagree) to +3 (completely agree).
...
Self-control was measured using the short form of the German Self-Control Scale [SCS-K-D; @Bertrams2009a] that comprises 13 items (e.g., “I am able to work effectively toward long-term goals”) with a 5-point Likert scale ranging from -2 (completely disagree) to +2 (completely agree), with comparably high reliability (Cronbach’s $\alpha>.80$, 7-week retest reliability $r_{tt}=.82$) [@Bertrams2009a].
Reappraisal and suppression was measured using the 10-item Emotion Regulation Questionnaire (ERQ) [@Abler2009].
The scale has items such as "When I’m faced with a stressful situation, I make myself think about it in a way that helps me stay calm" (reappraisal) and "I keep my emotions to myself" (suppression), which are answered on a 7-point Likert scale ranging from 1 (strongly disagree) to 7 (strongly agree), and has acceptable to high internal consistency (Cronbach’s $\alpha>.75$) [@Preece2019].
Work satisfaction was assessed using the Allgemeine Arbeitszufriedenheit questionnaire [@Fischer2014] with items such as "Sometimes I feel like my work doesn't really matter in my firm", which are answered on a 5-point Likert scale from 1 to 5 and different anchors depending on the item.
The scale has a split-half reliability of $r_{tt}=.95$ [@Fischer2014].
...
Due to a technical error during survey setup, the coping style data of the Erfurter Belastungsinventar [@BoehmKasper2001] could not be used, so we could not replicate the mediation of NFC and reduced personal efficacy by active and passive coping.
...
Correlations between variables were classified according the scheme by Gignac and Szodorai [-@Gignac2016], which recommends .10, .20, and .30 to be considered small, typical, and relatively large, respectively.

### Replication of Grass et al. (2018) in a teacher sample
...
The latent factor NFC was computed by subjecting the NCS items to a parcelling procedure [@Little2002], a method that is used in SEM when only relations between but not within constructs are of interest.
...
Using traditional cut-off values [@Nunnally1994], the Cronbach's Alpha of the three demand-resource-ratios in this sample were *acceptable*.
...
As expected, the MBI score showed a large positive correlation with *demands too high* (`r cor_apa(cor.test(score_data$mbi, score_data$dth, method = "spearman", exact = FALSE), format = "rmarkdown", print = FALSE)`) and a large negatve one with *demand-resource-fit* (`r cor_apa(cor.test(score_data$mbi, score_data$drf, method = "spearman", exact = FALSE), format = "rmarkdown", print = FALSE)`).
...
```{r objective1table, echo=FALSE, message=FALSE, warning=FALSE, caption="Results of the replication of Grass et al. (2018) in a teacher sample"}

  replic_table <-
    knitr::kable(replic_table,
        caption = "Results of the replication of Grass et al. (2018) in a teacher sample.",
        booktabs = T, # no vertical lines
        escape = F, # correctly print special characters)
        align = c("l",rep("r",7))) %>% # right aligns the text in every numerical column
    kableExtra::kable_styling(latex_options = c("striped", "scale_down")) %>%
    kableExtra::pack_rows("Direct Effects", 1, 7) %>% # group the rows and add a header in between
    kableExtra::pack_rows("Indirect Effects", 8, 10) %>%
    kableExtra::pack_rows("Total Effect", 11, 11) %>%
    kableExtra::footnote(general = "$B$ = unstandardized regression coefficient, \\beta = standardized regression coefficient, $CI$ = confidence interval, NFC = Need for Cognition, RPE = reduced personal efficacy subscale of the Maslach Burnout Inventory, $SE$ = standard error.",
             escape = F, # correctly print special characters
             threeparttable = T)# line breaks in the footnote
...
We could replicate a positive association of NFC and self control ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "a1"], digits = 2)`, `r rp(fit_summ$PE$pvalue[fit_summ$PE$label == "a1"])`), and a negative association of habitual reappraisal and *reduced personal efficacy* ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "b2"], digits = 2)`, `r rp(fit_summ$PE$pvalue[fit_summ$PE$label == "b2"])`).
However, we could neither replicate the effect of NFC on reappraisal ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "a2"], digits = 2)`, `r rp(fit_summ$PE$pvalue[fit_summ$PE$label == "a2"])`), nor the indirect effect of NFC on *reduced personal efficacy* via reappraisal ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "Indirect2"], digits = 2)`, `r rp(fit_summ$PE$pvalue[fit_summ$PE$label == "Indirect2"])`).
Furthermore, even though NFC and *reduced personal efficacy* were both associated with self-control, the indirect effect of NFC on *reduced personal efficacy* via self control did not reach significance ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "Indirect1"], digits = 2)`, `r rp(fit_summ$PE$pvalue[fit_summ$PE$label == "Indirect1"])`).
Additionally, NFC was negatively associated with habitual use of suppression ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "a3"], digits = 2)`, `r rp(fit_summ$PE$pvalue[fit_summ$PE$label == "a3"])`), which was not the case in the study by Grass et al. [-@Grass2018].

```{r objective1plot, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="H", fig.cap="Standardized regression coefficients in the replication of Grass et al. (2018) in a teacher sample. * $p<.05$, ** $p<.01$."}
...
However, because of the positive association of years spent teaching and self control ($\beta=$ `r round(fityears_summ$PE$std.all[fityears_summ$PE$label == "y"], digits = 2)`, `r rp(fityears_summ$PE$pvalue[fityears_summ$PE$label == "y"])`), the indirect path leading from NFC and years spent teaching via self control to *reduced personal efficacy* reached significance in this model ($\beta=$ `r round(fityears_summ$PE$std.all[fityears_summ$PE$label == "Indirect1"], digits = 2)`, `r rp(fityears_summ$PE$pvalue[fityears_summ$PE$label == "Indirect1"])`).
Therefore, the total effect also increased slightly, compared to the first model ($\beta=$ `r round(fityears_summ$PE$std.all[fityears_summ$PE$label == "Total"], digits = 2)`, `r rp(fityears_summ$PE$pvalue[fityears_summ$PE$label == "Total"])`).

```{r objective2table, echo=FALSE, message=FALSE, warning=FALSE, caption="Results of the demand-resource-ratio model."}

  drrmodel_table <-
    knitr::kable(drr_table,
    ...
    kableExtra::pack_rows("Indirect Effects", 8, 10) %>%
    kableExtra::pack_rows("Total Effect", 11, 11) %>%
    kableExtra::footnote(general = "$B$ = unstandardized regression coefficient, $\\beta$ = standardized regression coefficient, CI = confidence interval, DTH = Demands Too High, DTL = Demands Too Low, DRF = Demand Resource Fit, MBI = Maslach Burnout Inventory, NFC = Need for Cognition, $SE$ = standard error.",
             escape = F, # correctly print special characters
             threeparttable = T)# line breaks in the footnote
```
...
NFC showed no direct association with the MBI score ($\beta=$ `r round(fit2_summ$PE$std.all[fit2_summ$PE$label == "c"], digits = 2)`, `r rp(fit2_summ$PE$pvalue[fit2_summ$PE$label == "c"])`), even though it was negatively correlated with the sum score and all subscales.
Instead, NFC showed indirect negative associations with the MBI score via lower scores in the latent variable *demands too high* ($\beta=$ `r round(fit2_summ$PE$std.all[fit2_summ$PE$label == "Indirect1"], digits = 2)`, `r rp(fit2_summ$PE$pvalue[fit2_summ$PE$label == "Indirect1"])`) and via higher scores in the latent variable *demand-resource-fit* ($\beta=$ `r round(fit2_summ$PE$std.all[fit2_summ$PE$label == "Indirect3"], digits = 2)`, `r rp(fit2_summ$PE$pvalue[fit2_summ$PE$label == "Indirect3"])`).
The latent variable *demands too low* was neither related to NFC ($\beta=$ `r round(fit2_summ$PE$std.all[fit2_summ$PE$label == "a2"], digits = 2)`, `r rp(fit2_summ$PE$pvalue[fit2_summ$PE$label == "a2"])`) nor to the MBI score ($\beta=$ `r round(fit2_summ$PE$std.all[fit2_summ$PE$label == "b2"], digits = 2)`, `r rp(fit2_summ$PE$pvalue[fit2_summ$PE$label == "b2"])`).
...
```{r exploratory1table, echo=FALSE, message=FALSE, warning=FALSE, caption="Results of the demand-resource-ratio model with reduced personal efficacy in place of the MBI score."}
  ...
  drr_rpe_table <-
    ...
    kableExtra::pack_rows("Indirect Effects", 8, 10) %>%
    kableExtra::pack_rows("Total Effect", 11, 11) %>%
    kableExtra::footnote(general = "$B$ = unstandardized regression coefficient, $\\beta$ = standardized regression coefficient, CI = confidence interval, DTH = Demands Too High, DTL = Demands Too Low, *demand-resource-fit* = Demand Resource Fit, RPE = Reduced Personal Efficacy subscale of the Maslach Burnout Inventory, NFC = Need for Cognition, $SE$ = standard error.",
             escape = F, # correctly print special characters
             threeparttable = T)# line breaks in the footnote
```
...
NFC showed no direct association with *reduced personal efficacy* ($\beta=$ `r round(fit_explor1_summ$PE$std.all[fit_explor1_summ$PE$label == "c"], digits = 2)`, `r rp(fit_explor1_summ$PE$pvalue[fit_explor1_summ$PE$label == "c"])`), but an indirect one via higher scores in the latent variable *demand-resource-fit* ($\beta=$ `r round(fit_explor1_summ$PE$std.all[fit_explor1_summ$PE$label == "Indirect3"], digits = 2)`, `r rp(fit_explor1_summ$PE$pvalue[fit_explor1_summ$PE$label == "Indirect3"])`).
And again, NFC was associated with lower scores in the latent variable *demands too high* ($\beta=$ `r round(fit_explor1_summ$PE$std.all[fit_explor1_summ$PE$label == "a1"], digits = 2)`, `r rp(fit_explor1_summ$PE$pvalue[fit_explor1_summ$PE$label == "a1"])`), but the latter did not mediate the relationship between NFC and *reduced personal efficacy* ($\beta=$ `r round(fit_explor1_summ$PE$std.all[fit_explor1_summ$PE$label == "Indirect1"], digits = 2)`, `r rp(fit_explor1_summ$PE$pvalue[fit_explor1_summ$PE$label == "Indirect1"])`) as it did with the MBI score in the previous model.
The latent variable *demands too low* was neither related to NFC ($\beta=$ `r round(fit_explor1_summ$PE$std.all[fit_explor1_summ$PE$label == "a2"], digits = 2)`, `r rp(fit_explor1_summ$PE$pvalue[fit_explor1_summ$PE$label == "a2"])`) nor to the MBI score ($\beta=$ `r round(fit_explor1_summ$PE$std.all[fit_explor1_summ$PE$label == "b2"], digits = 2)`, `r rp(fit_explor1_summ$PE$pvalue[fit_explor1_summ$PE$label == "b2"])`).
...
Then we modified the structure of the model using the *modindices()*-function in *lavaan* in order to increase the goodness-of-fit indices within the framework of contentually meaningful variable relationships.
...
Years spent teaching was associated with higher self control ($\beta=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$label == "ys"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$label == "ys"])`) and higher Covid burden ($\beta=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$label == "yc"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$label == "yc"])`) but not with NFC.
NFC covaried with self control ($\sigma_{NFC,scs}=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$lhs == "NFC" & fit_explor2_summ$PE$op == "~~" & fit_explor2_summ$PE$rhs == "scs"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$lhs == "NFC" & fit_explor2_summ$PE$op == "~~" & fit_explor2_summ$PE$rhs == "scs"])`) and Covid burden ($\sigma_{NFC,covb}=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$lhs == "NFC" & fit_explor2_summ$PE$op == "~~" & fit_explor2_summ$PE$rhs == "covb"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$lhs == "NFC" & fit_explor2_summ$PE$op == "~~" & fit_explor2_summ$PE$rhs == "covb"])`), but not with years spent teaching (`r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$lhs == "NFC" & fit_explor2_summ$PE$op == "~~" & fit_explor2_summ$PE$rhs == "years"])`).
In turn, NFC was associated with higher *demand-resource-fit* scores ($\beta=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$label == "ndrf"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$label == "ndrf"])`) and lower *demands too high* scores ($\beta=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$label == "ndth"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$label == "ndth"])`) but not directly with any of the two MBI subscales.
*Demand-resource-fit* scores fully mediated the negative association of NFC and self control with *reduced personal efficacy* (indirect effect $\beta=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$label == "Indirect1"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$label == "Indirect1"])`), which was also true for *demands too high* scores and *emotional exhaustion*, but *demands too high* also partially mediated between Covid burden and *emotional exhaustion* (indirect effect $\beta=$ `r round(fit_explor2_summ$PE$std.all[fit_explor2_summ$PE$label == "Indirect2"], digits = 2)`, `r rp(fit_explor2_summ$PE$pvalue[fit_explor2_summ$PE$label == "Indirect2"])`).
...
To achieve this, we replicated findings of mediators between Need for Cognition and reduced personal efficacy in a teacher sample, and extended the analysis to the role of different ratios of demands and resources in burnout using latent variable models.
...
While the mediation model had good fit, not all patterns were similar to the original study: NFC and self-control had a medium positive association, and reappraisal and *reduced personal efficacy* had a small negative association, but NFC and reappraisal were not associated.
There was, however, a small negative association between self-control and *reduced personal efficacy*, and between NFC and suppression.

NFC had a direct, medium negative effect on *reduced personal efficacy*, but this relationship was not mediated by any other variable.
Only when the amount of teaching experience was included as a predictor of self-control next to NFC, a very small indirect effect via self-control reached significance, indicating that teachers with high NFC and more years of teaching experience have higher self-control and, consequently, lower *reduced personal efficacy*.
...
The participants in the study by Grass et al. [-@Grass2018] were student teachers with a mean age of 25.5 years, while the majority of the current sample was between 40 and 59 years old.
...
## Exploratory analyses
### Demand-resource-ratio model with subscale reduced personal efficacy
...
Our study showed that self-control mediated between NFC and reduced personal efficacy when teaching experience was being taken into account.
Contrary to prior studies, neither habitual use of reappraisal nor use of suppression mediated between NFC and reduced personal efficacy.
...
\subsection{Items used to assess Covid burden}
...
\subsection{Results when excluding the outlier with very high MBI scores and very low NFC scores}

```{r outliercorrelationstable, echo=FALSE, message=FALSE, warning=FALSE, caption="Correlations and internal consistencies"}

  outliercorr_table <-
  knitr::kable(outliercorrelations,
      caption = "Spearman correlations and internal consistencies of the questionnaire scores.",
      booktabs = TRUE, # no vertical lines
      escape = FALSE, # correctly print special characters
      align = rep("l",14)) %>% # left aligns the text in every column
...
```{r outlierobjective1table, echo=FALSE, message=FALSE, warning=FALSE, caption="Results of the replication of Grass et al. (2018) in a teacher sample"}

  outlierreplic_table <-
    knitr::kable(outlierreplic_table,
        caption = "Results of the replication of Grass et al. (2018) in a teacher sample.",
        booktabs = T, # no vertical lines
        escape = F, # correctly print special characters)
        align = c("l",rep("r",7))) %>% # right aligns the text in every numerical column
    ...
    kableExtra::pack_rows("Indirect Effects", 8, 10) %>%
    kableExtra::pack_rows("Total Effect", 11, 11) %>%
    kableExtra::footnote(general = "$B$ = unstandardized regression coefficient, $\\beta$ = standardized regression coefficient, CI = confidence interval, NFC = Need for Cognition, RPE = reduced personal efficacy subscale of the Maslach Burnout Inventory, $SE$ = standard error, $N=179$.",
             escape = F, # correctly print special characters
             threeparttable = T)# line breaks in the footnote
...
```{r outlierobjective2table, echo=FALSE, message=FALSE, warning=FALSE, caption = "Results of the demand-resource-ratio model."}
  ...
  outlierdrrmodel_table <-
    ...
    kableExtra::pack_rows("Indirect Effects", 8, 10) %>%
    kableExtra::pack_rows("Total Effect", 11, 11) %>%
    kableExtra::footnote(general = "$B$ = unstandardized regression coefficient, $\\beta$ = standardized regression coefficient, CI = confidence interval, DTH = Demands Too High, DTL = Demands Too Low, DRF = Demand Resource Fit, MBI = Maslach Burnout Inventory, NFC = Need for Cognition, $SE$ = standard error, $N=179$.",
             escape = F, # correctly print special characters
             threeparttable = T)# line breaks in the footnote
```
...
\subsection{Replication of Grass et al. (2018) when including years spent teaching}

```{r objective1yearsplot, echo=FALSE, message=FALSE, warning=FALSE, fig.pos="H", fig.cap="Standardized regression coefficients in the replication of Grass et al. (2018) when including years spent teaching. * $p<.05$, ** $p<.01$, $N=180$."}
...
```{r objective1yearstable, echo=FALSE, message=FALSE, warning=FALSE, caption = "Results of the replication of Grass et al. (2018) when including years spent teaching."}
  ...
  replic_years_table <-
    knitr::kable(replic_years_table,
        caption = "Results of the replication of Grass et al. (2018) when including years spent teaching.",
        booktabs = T, # no vertical lines
        escape = F, # correctly print special characters
        align = c("l",rep("r",7))) %>% # right aligns the text in every numerical column
    kableExtra::kable_styling(latex_options = c("striped", "scale_down")) %>%
    kableExtra::pack_rows("Direct Effects", 1, 8) %>% # group the rows and add a header in between
    kableExtra::pack_rows("Indirect Effects", 9, 11) %>%
    kableExtra::pack_rows("Total Effect", 12, 12) %>%
    kableExtra::column_spec(1, width = "5cm") %>%
    kableExtra::footnote(general = "$B$ = unstandardized regression coefficient, $\\beta$ = standardized regression coefficient, CI = confidence interval, NFC = Need for Cognition, RPE = reduced personal efficacy subscale of the Maslach Burnout Inventory, $SE$ = standard error, $N=180$.",
             escape = F, # correctly print special characters
             threeparttable = T)# line breaks in the footnote
```
...
\subsection{Exploratory model with all relevant variables}

```{r explor2table, echo=FALSE, message=FALSE, warning=FALSE, caption = "Results of the exploratory model with Covid burden."}

  explor2_table <-
    knitr::kable(explor2_table,
        caption = "Results of the exploratory model with Covid burden.",
        booktabs = T, # no vertical lines
        escape = F, # correctly print special characters)
        align = c("l",rep("r",7))) %>% # right aligns the text in every numerical column
    kableExtra::kable_styling(latex_options = c("striped", "scale_down"), font_size = 11) %>%
    kableExtra::pack_rows("Direct Effects", 1, 10) %>% # group the rows and add a header in between
    kableExtra::pack_rows("Indirect Effects", 11, 12) %>%
    kableExtra::pack_rows("Total Effect", 13, 13) %>%
    kableExtra::column_spec(1, width = "5cm") %>%
    kableExtra::footnote(general = "$B$ = unstandardized regression coefficient, $\\beta$ = standardized regression coefficient, CI = confidence interval, COVB = Covid Burden, DTH = Demands Too High, DRF = Demand Resource Fit, MBI = Maslach Burnout Inventory, NFC = Need for Cognition, SCS = Self Control Scale, $SE$ = standard error, Years = Years spent teaching, $N=180$.",
             escape = F, # correctly print special characters
             threeparttable = T)# line breaks in the footnote
```
