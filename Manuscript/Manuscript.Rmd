...
```{r import, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # If the rest of the files follow the structure of the Github project, everything will work
...
## Material
All questionnaires were used in their German form. Burnout was assessed using the 21-item Maslach Burnout Inventory (MBI) [@Buessing1992], NFC was assessed using the 16-item Need for Cognition Scale (NCS) [@Bless1994], self-control was assessed using the 13-item Self-Control Scale (SCS) [@Bertrams2009a], reappraisal and suppression were assessed using the 10-item Emotion Regulation Questionnaire (ERQ) [@Abler2009], work satisfaction was assessed using the Allgemeine Arbeitszufriedenheit questionnaire (AAZ) [@Fischer2014].
...
```{r splitandrecode, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  mbi <- data[,10:30]             # Maslach Burnout Inventory
  erq <- data[,31:40]             # Emotion Regulation Questionnaire
  scs <- data[,41:53]             # Self Control Scale
  ws <- data[,54:90]              # Work Satisfaction
  ncs <- data[,c(91:98,100:107)]  # Need for Cognition Scale (without the dummy item)
  covb <- data[,108:118]          # Covid Burden Items
  ...
  dth <- dth - 3                                      # Recode all from 1-5 to to -2-+2
  dtl <- 6 - dtl                                      # Recode from 1-5 to to 5-1
  dtl <- dtl - 3                                      # Recode all from 1-5 to to -2-+2
  drf <- drf -3                                       # Recode all from 1-5 to to -2-+2
...
```{r parcelling, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  remove(ncs_load,ncs_pca) # remove unnecessary objects
...
# Create data frame to feed the values of every subject into
  ...
  for (i in 1:nrow(data)) {
    score_data[i,c(2:ncol(score_data))] <- c(rowSums(mbi[i,]),
  ...
  multicorrplot <- ggpairs(score_data[,c(2,3,4,5,9,10,15,19,23,27)], lower = list(continuous = my_fn))

  # Remove the outlier
  score_data <- score_data[score_data$mbi < 100, ]
...
```{r descriptives, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # Tests for normal distribution
    ...
    isnorm_test <- mvn(score_data[c("mbi","mbi_ee","mbi_dp","mbi_rpe","erq","erq_supp","erq_reap","scs","ncs","covb")],
  ...
  # Descriptive statistics
    ...
    desc_met <- data.frame(value = c("mean","sd","skewness","kurtosis","normality"),
                           mbi = rep(0,5),
                           mbi_ee = rep(0,5), mbi_dp = rep(0,5), mbi_rpe = rep(0,5),
                           erq = rep(0,5),
                           erq_supp = rep(0,5), erq_reap = rep(0,5),
                           scs = rep(0,5),
                           ncs = rep(0,5),
                           covb = rep(0,5))
    
    desc_met[1,2:ncol(desc_met)] <- isnorm_test$Descriptives$Mean
    desc_met[2,2:ncol(desc_met)] <- isnorm_test$Descriptives$Std.Dev
    desc_met[3,2:ncol(desc_met)] <- isnorm_test$Descriptives$Skew
    desc_met[4,2:ncol(desc_met)] <- isnorm_test$Descriptives$Kurtosis
    desc_met[5,2:ncol(desc_met)] <- isnorm_test$univariateNormality$Normality
...
```{r correlations, echo=FALSE, message=FALSE, warning=FALSE}
    ...
    # correlate variables, including the demand resource ratios
    corr_with_drr <- correlation_matrix(score_data[c("mbi","mbi_ee","mbi_dp","mbi_rpe","erq","erq_supp","erq_reap",
    ...
    # correlate variables without demand resource ratios (for table in manuscript)
    corr_no_drr <- correlation_matrix(score_data[c("mbi","mbi_ee","mbi_dp","mbi_rpe","erq","erq_supp","erq_reap",
                                                    "scs","ncs","covb")],
                                       type = "spearman",
                                       use = "lower",
    ...
    diag(corr_no_drr) <- consist_diag
    
    # set row and column names for better table printing
    row.names(corr_no_drr) <- c("MBI","MBI EE","MBI DP","MBI RPE","ERQ","ERQ S","ERQ R","SCS","NFC","COV")
    colnames(corr_no_drr) <- c("MBI","MBI EE","MBI DP","MBI RPE","ERQ","ERQ S","ERQ R","SCS","NFC","COV")
    ...
    # create three arrays with logical indicators of the significance levels
    corr_threestars <- corr_pvalues %>%
      as_tibble() %>%
      mutate(across(.cols = everything(),.fns = ~ ifelse(.x == "***", TRUE, FALSE)))
    corr_twostars <- corr_pvalues %>%
      as_tibble() %>%
      mutate(across(.cols = everything(),.fns = ~ ifelse(.x == "**", TRUE, FALSE)))
    corr_onestar <- corr_pvalues %>%
      as_tibble() %>%
      mutate(across(.cols = everything(),.fns = ~ ifelse(.x == "*", TRUE, FALSE)))
...
```{r desc_table, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  kbl(corr_no_drr,
      ...
      align = rep("r",11)) %>% # right aligns the text in every column except for the first
    kable_styling(latex_options = c("striped", "scale_down")) %>%
    column_spec(2, italic = corr_onestar$mbi|corr_twostars$mbi, bold = corr_threestars$mbi|corr_twostars$mbi) %>%
    column_spec(3, italic = corr_onestar$mbi_ee|corr_twostars$mbi_ee, bold = corr_threestars$mbi_ee|corr_twostars$mbi_ee) %>%
    column_spec(4, italic = corr_onestar$mbi_dp|corr_twostars$mbi_dp, bold = corr_threestars$mbi_dp|corr_twostars$mbi_dp) %>%
    column_spec(5, italic = corr_onestar$mbi_rpe|corr_twostars$mbi_rpe, bold = corr_threestars$mbi_rpe|corr_twostars$mbi_rpe) %>%
    column_spec(6, italic = corr_onestar$erq|corr_twostars$erq, bold = corr_threestars$erq|corr_twostars$erq) %>%
    column_spec(7, italic = corr_onestar$erq_supp|corr_twostars$erq_supp, bold = corr_threestars$erq_supp|corr_twostars$erq_supp) %>%
    column_spec(8, italic = corr_onestar$erq_reap|corr_twostars$erq_reap, bold = corr_threestars$erq_reap|corr_twostars$erq_reap) %>%
    column_spec(9, italic = corr_onestar$scs|corr_twostars$scs, bold = corr_threestars$scs|corr_twostars$scs) %>%
    column_spec(10, italic = corr_onestar$ncs|corr_twostars$ncs, bold = corr_threestars$ncs|corr_twostars$ncs) %>%
    column_spec(11, italic = corr_onestar$covb|corr_twostars$covb, bold = corr_threestars$covb|corr_twostars$covb) %>%
    footnote(general = "MBI = Maslach Burnout Inventory, MBI EE = Emotional exhaustion subscale, MBI DP = Depersonalisation subscale, MBI RPE = Reduced personal efficacy subscale, ERQ = Emotion Regulation Questionnaire, ERQ S = Suppression subscale, ERQ R = Reappraisal subscale, SCS = Self-Control Scale, NFC = Need for Cognition, COV = Covid-19 Burden. \\\\textit{N} = 179. Italics indicate \\\\textit{p} < .05, italics and bold indicate \\\\textit{p} < .01, bold indicates \\\\textit{p} < .001. Diagonal is Cronbach's Alpha and (in brackets) MacDonald's Omega.",
             escape = F, # correctly print special characters
...
```{r objective1, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
  ...
  # Include covariance of mediators in the model
    scs ~~ erq_reap
    scs ~~ erq_supp
    erq_reap ~~ erq_supp'
...
```{r objective1_plot, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # (This is just okay, it is not the final version, e.g. there is no path from NCS to MBI yet)

  fit_plot <- semPaths(fit, what="par", # display the parameters on the arrows
                       nodeLabels = c("Self-Control", "Reappraisal", "Suppression", "Burnout", "Need for Cognition"),
                       layout = matrix(data = c(0,0,0,1,-1,0.5,0,-0.5,0,0), nrow = 5, ncol = 2), # manually set the node locations
                       sizeMan = 20, sizeMan2 = 7, shapeMan = "rectangle", label.norm = TRUE, # specify node properties
                       edge.label.cex = 0.8, # size specifications
                       fade = FALSE, # do not make insignificant associations invisible
                       theme = "colorblind" # select appropriate colour scheme
                       )
```
...
```{r objective2_plot, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  fit2_plot <- semPaths(fit2, what = "par",
                        ...
                        edge.label.cex = 0.81, # size of the parameters
                        edge.label.color = "black", edge.label.bg = FALSE, # color and background of the parameters
                        posCol = "#f0a543", negCol = "#5a99d1" # color of arrows
  )
```
