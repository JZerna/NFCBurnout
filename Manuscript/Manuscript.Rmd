...
We could replicate a positive association of NFC and self control ($\beta = $`r round(fit_summ$PE$est[fit_summ$PE$label == "a1"], digits = 2)`, $p = $`r round(fit_summ$PE$pvalue[fit_summ$PE$label == "a1"], digits = 3)`), and a negative association of habitual reappraisal and reduced personal efficacy ($\beta = $`r round(fit_summ$PE$est[fit_summ$PE$label == "b2"], digits = 2)`, $p = $`r round(fit_summ$PE$pvalue[fit_summ$PE$label == "b2"], digits = 3)`).
However, we could neither replicate the effect of NFC on reappraisal ($\beta = $`r round(fit_summ$PE$est[fit_summ$PE$label == "b1"], digits = 2)`, $p = $`r round(fit_summ$PE$pvalue[fit_summ$PE$label == "b1"], digits = 3)`), nor the indirect effect of NFC on reduced personal efficacy via reappraisal ($\beta = $`r round(fit_summ$PE$est[fit_summ$PE$label == "Indirect2"], digits = 2)`, $p = $`r round(fit_summ$PE$pvalue[fit_summ$PE$label == "Indirect2"], digits = 3)`).
Instead, we found a significant indirect effect of NFC on reduced personal efficacy via self control ($\beta = $`r round(fit_summ$PE$est[fit_summ$PE$label == "Indirect1"], digits = 2)`, $p = $`r round(fit_summ$PE$pvalue[fit_summ$PE$label == "Indirect1"], digits = 3)`).
Additionally, NFC was negative associated with habitual use of suppression ($\beta = $`r round(fit_summ$PE$est[fit_summ$PE$label == "c1"], digits = 2)`, $p = $`r round(fit_summ$PE$pvalue[fit_summ$PE$label == "c1"], digits = 3)`), which was not the case in the study by Grass et al. [-@Grass2018].
