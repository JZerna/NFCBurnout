...
```{r objective1_table, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  fit_table <- data.frame("Path" = c("NFC $\\rightarrow$ Self Control", "NFC $\\rightarrow$ Reappraisal", "NFC $\\rightarrow$ Suppression",
  ...
  fit_table[2:ncol(fit_table)] <- format(round(fit_table[2:ncol(fit_table)], digits = 3), nsmall = 2)
...
Standardized estimates are displayed in **Figure**, total, direct, and indirect effects are listed in **Table**.
...
Additionally, NFC was negatively associated with habitual use of suppression ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "c1"], digits = 2)`, $p=$ `r round(fit_summ$PE$pvalue[fit_summ$PE$label == "c1"], digits = 3)`), which was not the case in the study by Grass et al. [-@Grass2018].
...
```{r objective1years_table, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  fityears_table <- data.frame("Path" = c("NFC $\\rightarrow$ Self Control", "Years spent teaching $\\rightarrow$ Self Control", "NFC $\\rightarrow$ Reappraisal", "NFC $\\rightarrow$ Suppression",
                                   "Self Control $\\rightarrow$ RPE", "Reappraisal $\\rightarrow$ RPE", "Suppression $\\rightarrow$ RPE", "NFC $\\rightarrow$ RPE",
  ...
  fityears_table[2:ncol(fityears_table)] <- format(round(fityears_table[2:ncol(fityears_table)], digits = 3), nsmall = 2)
...
```
```{r fit2_table, echo=FALSE, message=FALSE, warning=FALSE}
  
  # put relevant data from the mediation into a table for reporting

  # put data into data frame

  fit2_table <- data.frame("Path" = c("NFC $\\rightarrow$ DTH", "NFC $\\rightarrow$ DTL", "NFC $\\rightarrow$ DRF", "NFC $\\rightarrow$ MBI",
  ...
  fit2_table[2:ncol(fit2_table)] <- format(round(fit2_table[2:ncol(fit2_table)], digits = 3), nsmall = 2)
...
Standardized path coefficients of the demand-resource model are illustrated in **Figure**, total, direct, and indirect effects are listed in **Table**.
...
```{r exploratory1, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  fit_explor1_summ <- summary(fit_explor1, fit.measures = TRUE, standardize = TRUE, rsquare = TRUE,
          estimates = TRUE, ci = TRUE)
```
```{r exploratory1_plot, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  fit_explor1_plot <- semPaths(fit_explor1, what = "std",
...
The total, direct, and indirect effects are listed in **Supplementary Table**.
...
```{r explor2_plot, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  fit_explor2_plot <- semPaths(fit_explor2, what = "std",
...
The final model is illustrated in **Figure**, the total, direct, and indirect effects are listed in **Supplementary Table**.
