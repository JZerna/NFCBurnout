...
abstract: |
  ...
  In the present study, we analyzed questionnaire data of teachers that had been collected in spring of 2020.
  ...
  Secondly, we computed a structural equation model to investigate whether NFC and burnout were associated via different ratios of demands and personal resources.
  The hypothesized model did not have good fit, but indicated that individuals high in NFC have lower burnout scores due to demands that either fit their resources or do not exceed them.
...
```{r correlations_table, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r objective1_table, echo=FALSE, message=FALSE, warning=FALSE}

  # put relevant data from the mediation into a table for reporting

  # put data into data frame

  Table3 <- data.frame("Path" = c("NFC $\\rightarrow$ Self Control", "NFC $\\rightarrow$ Reappraisal", "NFC $\\rightarrow$ Suppression",
                                   "Self Control $\\rightarrow$ RPE", "Reappraisal $\\rightarrow$ RPE", "Suppression $\\rightarrow$ RPE", "NFC $\\rightarrow$ RPE",
                                   "NFC $\\rightarrow$ RPE via Self Control", "NFC $\\rightarrow$ RPE via Reappraisal", "NFC $\\rightarrow$ RPE via Suppression",
                                   "Total Effect"),
  ...
  Table3[2:ncol(Table3)] <- format(round(Table3[2:ncol(Table3)], digits = 3), nsmall = 2)
...
```{r objective1_plot, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%"}
  ...
  Figure1 <- semPaths(fit, what="std", # display the parameters on the arrows
...
However, we could neither replicate the effect of NFC on reappraisal ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "b1"], digits = 2)`, $p=$ `r round(fit_summ$PE$pvalue[fit_summ$PE$label == "b1"], digits = 3)`), nor the indirect effect of NFC on reduced personal efficacy via reappraisal ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "Indirect2"], digits = 2)`, $p=$ `r round(fit_summ$PE$pvalue[fit_summ$PE$label == "Indirect2"], digits = 3)`).
Furthermore, even though NFC and self control and reduced personal efficacy and self control were associated, the indirect effect of NFC on reduced personal efficacy via self control did not reach significance ($\beta=$ `r round(fit_summ$PE$std.all[fit_summ$PE$label == "Indirect1"], digits = 2)`, $p=$ `r round(fit_summ$PE$pvalue[fit_summ$PE$label == "Indirect1"], digits = 3)`).
...
```{r objective1years_table, echo=FALSE, message=FALSE, warning=FALSE}

  # put relevant data from the mediation into a table for reporting

  # put data into data frame

  Table4 <- data.frame("Path" = c("NFC $\\rightarrow$ Self Control", "Years spent teaching $\\rightarrow$ Self Control", "NFC $\\rightarrow$ Reappraisal", "NFC $\\rightarrow$ Suppression",
                                   "Self Control $\\rightarrow$ RPE", "Reappraisal $\\rightarrow$ RPE", "Suppression $\\rightarrow$ RPE", "NFC $\\rightarrow$ RPE",
                                   "NFC and years spent teaching $\\rightarrow$ RPE via Self Control", "NFC $\\rightarrow$ RPE via Reappraisal", "NFC $\\rightarrow$ RPE via Suppression",
                                   "Total Effect"),
  ...
  Table4[2:ncol(Table4)] <- format(round(Table4[2:ncol(Table4)], digits = 3), nsmall = 2)

  replicationyears_table <-
    kbl(Table4,
        booktabs = T, # no vertical lines
        escape = F, # correctly print special characters)
        align = c("l",rep("r",7))) %>% # right aligns the text in every numerical column
...
```{r objective1years_plot, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%"}
  ...
  Figure2 <- semPaths(fityears, what="std", # display the parameters on the arrows
...
Standardized estimates are displayed in Figure 2, total, direct, and indirect effects are listed in Table 4.
The associations between NFC, self control, reappraisal, suppression, and reduced personal efficacy were almost identical to the model first model.
...
```{r objective2_plot, echo=FALSE, message=FALSE, warning=FALSE, out.width="100%"}
  ...
  Figure3 <- semPaths(fit2, what = "std",
...
```{r objective2_table, echo=FALSE, message=FALSE, warning=FALSE}
  
  # put relevant data from the mediation into a table for reporting

  # put data into data frame

  Table5 <- data.frame("Path" = c("NFC $\\rightarrow$ DTH", "NFC $\\rightarrow$ DTL", "NFC $\\rightarrow$ DRF", "NFC $\\rightarrow$ MBI",
                                      "DTH $\\rightarrow$ MBI", "DTL $\\rightarrow$ MBI", "DRF $\\rightarrow$ MBI",
                                      "NFC $\\rightarrow$ MBI via DTH", "NFC $\\rightarrow$ MBI via DTL", "NFC $\\rightarrow$ MBI via DRF",
                                      "Total Effect"),
  ...
  Table5[2:ncol(Table5)] <- format(round(Table5[2:ncol(Table5)], digits = 3), nsmall = 2)
...
Standardized path coefficients of the demand-resource model are illustrated in Figure 3, total, direct, and indirect effects are listed in Table 5.
...
```{r exploring_badfit, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r exploratory1_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hide'}
  ...
  FigureS1 <- semPaths(fit_explor1, what = "std",
...
```{r exploratory1_table, echo=FALSE, message=FALSE, warning=FALSE}
  
  # put relevant data from the mediation into a table for reporting

  # put data into data frame

  TableS1 <- data.frame("Path" = c("NFC $\\rightarrow$ DTH", "NFC $\\rightarrow$ DTL", "NFC $\\rightarrow$ DRF", "NFC $\\rightarrow$ RPE",
                                      "DTH $\\rightarrow$ RPE", "DTL $\\rightarrow$ RPE", "DRF $\\rightarrow$ RPE",
                                      "NFC $\\rightarrow$ RPE via DTH", "NFC $\\rightarrow$ RPE via DTL", "NFC $\\rightarrow$ RPE via DRF",
                                      "Total Effect"),
  ...
  TableS1[2:ncol(TableS1)] <- format(round(TableS1[2:ncol(TableS1)], digits = 3), nsmall = 2)
...
The path coefficients are illustrated in Supplementary Figure S1, total, direct, and indirect effects are listed in **Supplementary Table**.
...
```{r exploratory2, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r explor2_plot, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  FigureS2 <- semPaths(fit_explor2, what = "std",
...
```{r explor2_table, echo=FALSE, message=FALSE, warning=FALSE}
  
  # put relevant data from the mediation into a table for reporting

  # put data into data frame

  TableS2 <- data.frame("Path" = c("Years $\\rightarrow$ COVB", "Years $\\rightarrow$ SCS",
                                   "COVB $\\rightarrow$ DTH", "SCS $\\rightarrow$ DTH", "NFC $\\rightarrow$ DTH",
                                   "SCS $\\rightarrow$ DRF", "NFC $\\rightarrow$ DRF",
                                   "DTH $\\rightarrow$ EE", "COVB $\\rightarrow$ EE", "DRF $\\rightarrow$ RPE",
                                   "NFC and Years $\\rightarrow$ RPE via SCS and DRF",
                                   "NFC and Years $\\rightarrow$ EE via SCS, COVB, and DTH",
                                   "Total Effect"),
  ...
  TableS2[2:ncol(TableS2)] <- format(round(TableS2[2:ncol(TableS2)], digits = 3), nsmall = 2)
...
Then we modified the structure of the model in order to increase the goodness-of-fit indices within the framework of contentually meaningful variable relationships.
The final model is illustrated in Supplementary Figure S2, the total, direct, and indirect effects are listed in **Supplementary Table**.
All fit indices suggest that the proposed model has good fit while the baseline model does not ($\chi^2=$ `r round(fit_explor2@test[["yuan.bentler.mplus"]]$stat, digits = 2)` ($p < .001$), $CFI=$ `r round(fit_explor2_summ[["FIT"]][["cfi.robust"]], digits = 2)`, $RMSEA=$ `r round(fit_explor2_summ[["FIT"]][["rmsea.robust"]], digits = 2)` (95% $CI$ [`r round(fit_explor2_summ[["FIT"]][["rmsea.ci.lower.robust"]], digits = 2)`,`r round(fit_explor2_summ[["FIT"]][["rmsea.ci.upper.robust"]], digits = 2)`]), $SRMR=$ `r round(fit_explor2_summ[["FIT"]][["srmr"]], digits = 2)`).
...
The questionnaire data had been acquired in the scope of a thesis on burnout in teachers.

## Replication of Grass et al. (2018)

While the mediation model, which replicated the findings of Grass et al. [-@Grass2018], had good fit, not all patterns were similar to the original study.
NFC and self-control were positively associated, and reappraisal and reduced personal efficacy were negatively related, but there was no association between NFC and reappraisal.
...
The higher self-control that comes with more teaching experience is not in line with findings of stable self-control over time [@Hay2006], but in line with other findings of fluctuations in self-control in young adults, reaching a low point between the age of 15 and 19 [@Olivia2019].
