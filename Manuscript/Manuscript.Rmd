...
header-includes   : \usepackage{tabu}
---
...
#install.packages("kableExtra") # for great tables
...
```{r correlations, echo=FALSE, message=FALSE, warning=FALSE}
    ...
    row.names(corr_no_drr) <- c("MBI Score","MBI EE","MBI DP","MBI RPE","ERQ","ERQ Suppression","ERQ Reappraisal","Self Control","NFC","Covid-19 Burden")
...
```{r desc_table, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}

  desc_table <-
  kbl(corr_no_drr,
      booktabs = T,
      align = rep("r",11)) %>%
    kable_styling(latex_options = "striped", position = "center", full_width = T, font_size = 10) %>%
...
```{r objective2, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE}
...
```
```{r objective2_troubleshooting, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  # Plot a big correlational matrix to check for non-linear associations that might have caused the bad model fit
  multicorrplot <- ggpairs(score_data[,c(2,3,4,5,9,10,15,19,23,27)], lower = list(continuous = my_fn))
  
  # Remove the outlier
  score_data <- score_data[score_data$id != 161, ]
